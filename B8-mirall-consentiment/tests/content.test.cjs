...
test("reflection, transfer, and commitment choices cover uncertainty", () => {
  assert.ok(Content.PREDICTIONS.length >= 3);
  assert.ok(Content.DIFFERENCE_REASONS.length >= 8);
  assert.deepEqual(
    Content.DISPOSITIONS.map(({ id }) => id),
    ["maintain", "revisit", "pending"],
  );
  assert.deepEqual(
    Content.SAME_DISPOSITIONS.map(({ id }) => id),
    ["maintain", "revisit", "pending"],
  );
  ...
  assert.equal(Content.EVIDENCE_CHOICES.length, 4);
